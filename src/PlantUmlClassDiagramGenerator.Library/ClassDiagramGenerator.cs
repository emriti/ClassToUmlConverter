...
namespace PlantUmlClassDiagramGenerator.Library
{
    public class ClassDiagramGenerator : CSharpSyntaxWalker
    {
        ...
        public override void VisitFieldDeclaration(FieldDeclarationSyntax node)
        {
            ...
            {
                var isTypesShouldInclude = IncludeTypes.Types.Any(t => t == type?.ToString()?.ToLower());
                Type fieldType = type.GetType();
                if (!_createAssociation || fieldType == typeof(PredefinedTypeSyntax) || fieldType == typeof(NullableTypeSyntax) || isTypeParameterField || isTypesShouldInclude)
                {
        ...
        public override void VisitPropertyDeclaration(PropertyDeclarationSyntax node)
        {
            ...
            var isTypesShouldInclude = IncludeTypes.Types.Any(t => t == node?.Type?.ToString()?.ToLower());

            if (!_createAssociation || type.GetType() == typeof(PredefinedTypeSyntax)  || type.GetType() == typeof(NullableTypeSyntax) || isTypeParameterProp || isTypesShouldInclude)
            {
                var modifiers = GetMemberModifiersText(node.Modifiers);
                var name = node.Identifier.ToString();
                ...
                // MyChange: remove accessor
                WriteLine($"{modifiers}{name} : {type.ToString()}");
            }
            else
            {
